...
def createReq(M, N, K, alpha, beta):
    ...
    #d = np.zeros(shape=(N, 1), dtype=np.float32)
    ...
    #libffCtx.kv.put('b', b)
    bBuf = kaas.bufferSpec('b', b.nbytes, const=True, ephemeral=False)

    #libffCtx.kv.put('c', c)
    cBuf = kaas.bufferSpec('c', c.nbytes, const=False, ephemeral=False)
