...
def getOnnx(inputPath, outputDir, inputShapeMap=None):
    ...
    #module.export_library((outputDir / 'lib').with_suffix(".so"))
    ...
    '''
    srcPath = outputDir / "source.cu"
    with open(srcPath, 'w') as f:
        f.write(cudaLib.get_source())

    meta_path = outputDir / "meta.ptx"
    print(meta_path)
    cudaLib.save(str(meta_path))

    meta = infbench.model.getOnnxInfo(model)
    with open((outputDir / 'extra').with_suffix(".json"), 'w') as f:
...
def getSsdMobilenet():
    ...
    with tvm.transform.PassContext(opt_level=3):
        module = relay.build(mod, tvm.target.cuda(), params=params)
    module.export_library(ssdDir / 'ssdMobilenet.so')
    ...
    with open(ssdDir / 'graph.json', 'w') as f:
        json.dump(meta, f)
    
    with open(ssdDir / 'graph.txt', 'w') as f:
        f.write(module.get_graph_json())
    paramPath = ssdDir / "params.pickle"
    with open(paramPath, 'wb') as f:
        pickle.dump({k: p.asnumpy() for k, p in module.params.items()}, f)

    srcPath = ssdDir / "source.cu"
    with open(srcPath, 'w') as f:
        f.write(cudaLib.get_source())

    meta_path = ssdDir / "meta.ptx"
    cudaLib.save(str(meta_path))
